            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 pl-2 group cursor-pointer">
                <div className="text-right">
                  <p className="text-xs font-bold text-white group-hover:text-white/80 transition-colors">
                    Marcus Vane
                  </p>
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <aside className="w-80 border-r border-[#262626] p-8 flex flex-col gap-8 bg-[#0D0D0D]">
              <div className="flex items-center justify-between">
                <h2 className="text-sm uppercase tracking-widest font-bold text-[#555]">
                  Categories
                </h2>
                <button
                  onClick={() => setShowModal(true)}
                  className="p-1.5 rounded-lg bg-[#161616] text-[#888] hover:text-white hover:bg-[#222] transition-colors"
                >
                  <Plus size={16} />
                </button>
              </div>
                    ...
                    <div className="flex items-center gap-3">
                      <span
                        className={
                          activeCategory === cat._id
                            ? "text-black"
                            : "text-[#555] group-hover:text-white transition-colors"
                        }
                      >
              ...
              <div className="mt-auto">
                <div className="p-5 rounded-2xl bg-linear-to-br from-[#161616] to-[#0D0D0D] border border-[#262626]">
                  <p className="text-[10px] uppercase tracking-widest text-[#555] mb-3 font-bold">
                    System Status
                  </p>
                  <div className="flex items-center gap-3">
            ...
            <main className="flex-1 overflow-y-auto p-10 bg-[#0B0B0B]">
              <div className="flex justify-between items-center mb-10">
                <div>
                  <h2 className="text-4xl font-black text-white mb-2 capitalize tracking-tight">
                    {activeCategoryData?.name || "Categories"}
                  </h2>
                  <p className="text-[#666] text-sm">
                    Curate and manage your premium selection for this category.
                  </p>
                </div>
                <button
                  onClick={() => navigate("/admin/addmenuitems")}
                  className="bg-white text-black px-6 py-3 rounded-full font-bold text-sm flex items-center gap-2 hover:bg-[#EEE] active:scale-95 transition-all shadow-xl shadow-white/5"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                {items.map((dish) => (
                  <div
                    key={dish.id}
                    className="group bg-[#121212] border border-[#262626] rounded-3xl overflow-hidden hover:border-[#444] transition-all duration-300"
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between pt-5 border-t border-[#262626]">
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <div
                              className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${dish.available ? "left-6" : "left-1"}`}
                            ></div>
                          </button>
                          <span
                            className={`text-[10px] font-bold uppercase tracking-widest ${dish.available ? "text-green-500" : "text-[#555]"}`}
                          >
...
const HeaderAction = ({ icon, dot }) => (
  <button className="p-2.5 rounded-xl bg-[#161616] border border-[#262626] text-[#888] hover:text-white hover:border-[#444] transition-all relative cursor-pointer">
    {icon}
    {dot && (
      <span className="absolute top-2 right-2 w-2 h-2 bg-white rounded-full border-2 border-[#161616]"></span>
    )}
  </button>
);
